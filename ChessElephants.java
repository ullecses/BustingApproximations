...
public class ChessElephants implements Runnable {
    ...
    public void run() {
        ...
        try {
            ...
            for (int i = 0; i < n; i++) {
                for (int j = 0; j < m; j++) {
                    matrix[i][j] = fs.nextInt(); // Заполняем матрицу элементами
                    //lines[i].add(j);
                    //columns[j].add(i);
    ...
    public static void main(String[] args) {
        new Thread(null, new ChessElephants(), "", 64 * 1024 * 1024).start();
